...
import styles from "./kiosk-marquee.module.css";
...
export default function KioskPage() {
  ...
  const resetTimer = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const acquireQueue = async (
    ...
    if (!idempotencyKey) {
      setState({
        type: "error",
        message:
          "Gagal membuat permintaan antrean.",
    ...
    try {
      ...
      try {
        response = await fetch(
          `${API_BASE_URL}/kiosk/queue-acquisitions?${params.toString()}`,
          {
            method: "POST",
            headers: {
              Accept: "application/json",
        ...
        <div className="mt-7 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div className="flex h-12 items-center">
            ...
            <div className="relative min-w-0 flex-1 overflow-hidden">
              <div
                className={`${styles.marquee} flex min-w-max items-center whitespace-nowrap py-3 text-sm font-medium text-muted-foreground`}
              >
                <span className="px-8">
